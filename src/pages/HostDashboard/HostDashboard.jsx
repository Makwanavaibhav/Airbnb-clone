...
import { ArrowLeft, Home, MapPin, IndianRupee, ImagePlus, Loader2, Menu, Trash2, Edit2, Calendar, Clock } from "lucide-react";
...
const HostDashboard = () => {
  ...
  const loadDrafts = () => {
    ...
    const loaded = idx.map(id => {
      try {
        return JSON.parse(localStorage.getItem(`listing_draft_${id}`) || "null");
      } catch { return null; }
    }).filter(Boolean);
    setDrafts(loaded);
  };
  ...
  const fetchTodayBookings = async () => {
    ...
    try {
      ...
      const res = await axios.get("http://localhost:5001/api/bookings/my-trips?status=upcoming", {
        headers: { Authorization: `Bearer ${token}` }
      });
      setBookings(res.data.trips || []);
    } catch(err) {
  ...
  return (
    <div className="min-h-screen bg-white">
      ...
      {/* Main Content Area - tall enough to require scrolling */}
      <div className="max-w-screen-xl mx-auto px-6 md:px-12 py-10 pb-32 flex flex-col items-center min-h-[calc(100vh-80px)]">
        ...
        {activeTab === "Today" ? (
          <>
            ...
            {loadingBookings ? (
              ...
              <div className="w-full max-w-2xl space-y-4">
                <h2 className="text-2xl font-semibold text-gray-900 mb-6">Your upcoming reservations</h2>
                {bookings.map(trip => {
                  const hotel = trip.hotelId || {};
                  const checkIn = new Date(trip.checkInDate);
                  const checkOut = new Date(trip.checkOutDate);
                  const formattedCheckIn = checkIn.toLocaleDateString("en-IN", { weekday: "short", day: "numeric", month: "long", year: "numeric" });
                  const formattedCheckOut = checkOut.toLocaleDateString("en-IN", { day: "numeric", month: "long", year: "numeric" });
                  return (
                    <div key={trip._id} className="flex items-start gap-5 p-5 border border-gray-200 rounded-2xl hover:shadow-md transition-shadow bg-white">
                      <div
                        className="w-20 h-20 rounded-xl bg-gray-200 object-cover flex-shrink-0 overflow-hidden cursor-pointer"
                      ...
                      <div className="flex-1">
                        <h3 className="font-semibold text-[17px] text-gray-900 mb-1">{hotel.title || hotel.location || "Booking"}</h3>
                        <p className="text-gray-500 text-sm mb-3">{hotel.location}</p>
                        <div className="flex items-center gap-2 text-sm text-gray-700 font-medium">
                          <Calendar className="w-4 h-4 text-[#FF385C]" />
                          <span>Check-in: <span className="font-semibold text-gray-900">{formattedCheckIn}</span></span>
                        </div>
                      ...
                      <span className={`px-3 py-1 rounded-full text-xs font-semibold ${trip.status === "confirmed" ? "bg-green-100 text-green-800" : "bg-gray-100 text-gray-600"}`}>
          ...
          <div className="w-full flex flex-col items-start mt-10">
            <div className="flex justify-between items-center w-full mb-8">
              ...
              <button
                onClick={() => { setResumeDraft(null); setShowListingForm(true); }}
            ...
            {loadingListings ? (
              ...
              <>
                  ...
                  <div className="w-full mb-8">
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                      {drafts.map(draft => (
                        <div
                          key={draft.draftId}
                          className="bg-white border-2 border-dashed border-amber-300 hover:border-amber-500 hover:shadow-md transition-all rounded-2xl overflow-hidden group cursor-pointer"
                        ...
                        >
                          <div className="relative aspect-[4/3] bg-amber-50 flex flex-col items-center justify-center">
                            ...
                            <button
                              onClick={(e) => { e.stopPropagation(); handleDeleteDraft(draft.draftId); }}
                ...
                {properties.length === 0 && drafts.length === 0 ? (
                  <div className="w-full bg-gray-50 py-20 rounded-2xl flex flex-col items-center text-center mt-4 border border-gray-100">
